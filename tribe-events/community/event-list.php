			...
			<tbody id="the-list"><tr>
				...
				while ( $events->have_posts() ) {
					...
					<tr>
						...
						<td><?php echo tribe( 'community.main' )->getEventStatusIcon( $post->post_status ); ?></td>
						<td>
						<?php
						...
						<div class="row-actions">
							...
							if ( $canEdit ) {
								echo tribe( 'community.main' )->getEditButton( $post, __( 'Edit', 'tribe-events-community' ), '<span class="edit wp-admin events-cal"> |', '</span> ' );
							}

							if ( $canDelete ) {
								echo tribe( 'community.main' )->getDeleteButton( $post );
							}
						...
						<td>
							...
							if ( tribe_has_organizer( $post->ID ) ) {
								...
								if ( current_user_can( 'edit_post', $organizer_id ) ) {
									echo '<a href="'. esc_url( tribe( 'community.main' )->getUrl( 'edit', $organizer_id, null, Tribe__Events__Main::ORGANIZER_POST_TYPE ) ) .'">'. tribe_get_organizer( $post->ID ) .'</a>';
								} else {
						...
						<td>
							...
							if ( tribe_has_venue( $post->ID ) ) {
								...
								if ( current_user_can( 'edit_post', $venue_id ) ) {
									echo '<a href="' . esc_url( tribe( 'community.main' )->getUrl( 'edit', $venue_id, null, Tribe__Events__Main::VENUE_POST_TYPE ) ) . '">'. tribe_get_venue( $post->ID ) .'</a>';
								} else {
						...
						<td>
							<?php echo esc_html( tribe_get_start_date( $post->ID, tribe( 'community.main' )->eventListDateFormat ) ) ?>
						</td>

						<td>
							<?php echo esc_html( tribe_get_end_date( $post->ID, tribe( 'community.main' )->eventListDateFormat ) ) ?>
						</td>
